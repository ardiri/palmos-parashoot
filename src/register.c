...
{
  const RectangleType rect      = {{0,147},{160,14}};
...
RegisterTerminate()
{
  ...
  // clean up memory
  if (gbls->adjustGame) MemPtrFree(gbls->adjustGame);
  MemPtrFree(gbls);
            ...
            GameAdjustmentType *adjustType) 
{
  ...
  switch (adjustType->adjustMode) 
  {
    case gameScoreCode:
         {
           ...
           for (i=1; (i < MAX_IDLENGTH) && (key[i] != '\0'); i++) {
             result -= (
                        ((gbls->adjustGame[index] & 0x96) << 8) | 
                         (gbls->adjustGame[index] & 0x69)
                       );
             result  = (((result & 0x7e7e) << 1) ^ ((result & 0x8181) >> 15));
    ...
    case gameHighScore:
         switch (prefs->game.parashoot.gameType) 
         {
           case GAME_A:
                // did the player beat (or equal) the GAME_A high score?
                if (prefs->game.gameScore >= prefs->game.highScore[0]) {
                  prefs->game.highScore[0] = prefs->game.gameScore;
                }
                else result = false;
                break;

           case GAME_B:
                // did the player beat (or equal) the GAME_B high score?
                if (prefs->game.gameScore >= prefs->game.highScore[1]) {
                  prefs->game.highScore[1] = prefs->game.gameScore;
                }
                else result = false;
                break;
